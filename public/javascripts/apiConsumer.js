...
$("#updateBtn").hide()
//methode principale de pagination
...
const createRow = (id,username,email,password,role) =>{
    return $("<tr/>").append(`<td>${id}</td><td>${username}</td><td>${email}</td><td>${password}</td><td>${role}</td>`)
...
function updateButtons(){
    //database 20  rows limit = 6
    //(5)*6 = 30 >= (26)
    if((currentPage+1)*limit>=(nbElements+limit)){
        $("#nextUser").hide() //hide the next button
    }else{
        $("#nextUser").show()
    }
    //page = 1 => hide the prev button
    if(currentPage == 1){
        $("#previousUser").hide()
    }else{
        $("#previousUser").show()
